...
async def _save_list_to_excel(
    ...
    try:
        ...
        logger.error(
            "Помилка збереження Excel файлу для користувача %s: %s",
            user_id,
            e,
            exc_info=True,
        )
...
async def process_and_save_list(
    ...
    for item in temp_list:
        ...
        if item.quantity <= available:
            in_stock_items.append(
                {"Артикул": product.артикул, "Кількість": item.quantity}
            )
            total_in_stock_sum += item.quantity * price
        else:
            if available > 0:
                in_stock_items.append(
                    {"Артикул": product.артикул, "Кількість": available}
                )
                total_in_stock_sum += available * price

            surplus_quantity = item.quantity - available
            surplus_items.append(
                {"Артикул": product.артикул, "Кількість": surplus_quantity}
            )
    ...
    main_list_path = await _save_list_to_excel(
        in_stock_items, user_id, department_id, total_in_stock_sum
    )
    surplus_list_path = await _save_list_to_excel(
        surplus_items, user_id, department_id, total_surplus_sum, "лишки_"
    )

    if main_list_path and in_stock_items:
        # Зберігаємо article_name для сумісності зі старими звітами
        db_items = [
            {"article_name": p.product.назва, "quantity": p.quantity} for p in temp_list
        ]
        await orm_add_saved_list(
            session, user_id, os.path.basename(main_list_path), main_list_path, db_items
        )
