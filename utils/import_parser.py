...
class ImportParser:
    ...
    def _clean_numeric(self, val: Any) -> float:
        ...
        s_val = str(val).replace(',', '.').replace(u'\xa0', '').strip()
        ...
        for idx, row in self.df.iterrows():
            ...
            try:
                ...
                parsed_items.append({
                    ...
                    "відділ": dept,
                    "група": group,
                    "кількість": str(qty_val), # Зберігаємо як стрінг для сумісності з БД
                ...
                self.validation_errors.append(f"Рядок {row_num} (Арт {final_article}): Помилка даних - {e}")
