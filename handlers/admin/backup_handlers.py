...
from utils.backup_utils import create_backup, get_backups_list, delete_old_backups
...
async def open_backups_menu(callback: CallbackQuery):
    ...
    if not backups:
        ...
        for b in backups[:5]:
            ...
            text += LEXICON.BACKUP_ITEM_TEMPLATE.format(
    ...
    try:
        await callback.message.edit_text(text, reply_markup=get_backups_menu_kb(backups))
...
async def handle_cleanup_backups(callback: CallbackQuery):
    ...
    await callback.answer(LEXICON.BACKUP_DELETED_COUNT.format(count=count), show_alert=True)
    await open_backups_menu(callback)
