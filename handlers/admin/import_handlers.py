...
async def proceed_with_import(message: Message, state: FSMContext, bot: Bot, is_after_force_save: bool = False):
    back_kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=LEXICON.BUTTON_BACK_TO_ADMIN_PANEL, callback_data="admin:main")]])
    text = LEXICON.IMPORT_PROMPT if not is_after_force_save else "Списки збережено. Надішліть файл."
...
async def start_import_handler(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    await callback.message.edit_text(LEXICON.ACTIVE_LISTS_BLOCK.format(users_info=users_info), reply_markup=get_admin_lock_kb(action='import'))
...
async def analyze_import_file(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        for idx, item in enumerate(items[:5], 1): # Перші 5
            preview_lines.append(f"{idx}. `{item['артикул']}` | {item['назва'][:20]}.. | {item['кількість']} шт")
...
async def confirm_import(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        # 3. Результат
        if 'error' in result:
            await callback.message.edit_text(LEXICON.IMPORT_ERROR.format(error=result['error']))
        else:
            ...
            await callback.message.answer(LEXICON.IMPORT_ASK_FOR_NOTIFICATION, reply_markup=get_notify_confirmation_kb())
