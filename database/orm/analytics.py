...
from database.models import Product, SavedList, SavedListItem, StockHistory, User
...
def get_products_dataframe(filter_type: str = "all") -> pd.DataFrame:
    ...
    with sync_session() as session:
        ...
        if filter_type == "active":
            query = query.where(Product.активний)
        elif filter_type == "inactive":
            query = query.where(not Product.активний)
        elif filter_type == "no_move":
            # Товари, що лежать без руху 3+ місяців
            query = query.where(Product.місяці_без_руху >= 3).order_by(
                Product.місяці_без_руху.desc()
            )
